...
- 兼容三种首期资源形态：
  ...
  - ZIP 技能包型：支持上传、展示、下载 ZIP，详情窗口主按钮为下载 ZIP，同时支持复制安装命令。
  - 单 Markdown 型：支持上传单个 `.md` 文件，详情窗口直接展示 Markdown 内容，支持下载 MD 和复制内容。
...
详情不使用右侧滑出的 Drawer。用户点击卡片后，在 Explore 右侧内容区打开布局内全屏详情窗口，保留左侧 Explore 导航和页面框架，详情视图覆盖列表内容区。详情窗口顶部提供返回列表按钮，支持滚动和复制反馈。

实现可以采用两种等价方式：

- 路由式详情：`/explore/skills/{slug}` 渲染同一 Explore 布局下的详情窗口。
- 状态式详情：`/explore/skills` 内部切换列表和详情状态，并同步 URL 查询参数。

首期推荐路由式详情，便于分享链接、刷新恢复和测试。

详情头部：

- icon、名称、版本、作者。
- 资源类型徽标。
- 审计状态信息，展示在详情窗口内，不出现在列表卡片上。
- 主操作按钮。
...
- `web/app/components/explore/skill-detail-view/*`
- `web/models/skill.ts`
...
- 复用 Explore 布局。
- 复用插件市场卡片密度和详情内容的信息层级。
- 详情窗口不复用 Drawer，使用布局内视图组件承载；后台编辑仍可复用 `@langgenius/dify-ui` 的 Button、Input、Dialog、Switch、Textarea 等组件。
