...
class WorkflowCommentService:
    ...
    def _build_mention_email_payloads(
        ...
        app_name = app_name_value if isinstance(app_name_value, str) and app_name_value else "July app"
        commenter_name_value = session.scalar(select(Account.name).where(Account.id == mentioner_id))
        commenter_name = (
            commenter_name_value if isinstance(commenter_name_value, str) and commenter_name_value else "July user"
        )
